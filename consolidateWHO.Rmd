...
if (file.exists(cache_path)) {
  ...
  geo_cache_wave1<- tibble(country = character(), city = character(), latitude = numeric(), longitude = numeric())
}
...
iterations <- 6
batch_size <- 500
pause_seconds <- 5  # polite pause between each batch
...
if (file.exists(second_wave_cache_file)) {
  ...
  geo_cache_wave2 <- geo_cache_wave2 %>% select(country, city, longitude, latitude)
} else {
...
# Wave 3 geocoding - manual review city names

library(tidygeocoder)

# Load manually revised file
geo_cache_manual_fixed <- read_csv("geo_cache_manual_review_fixed.csv", show_col_types = FALSE)

# Filter only revised rows
...
geo_results_wave3 <- geo_results_wave3 %>% select (country, city = city_fixed, latitude = latitude...7, longitude = longitude...8)  

print(geo_results_wave3)

# Load previous success (second wave)
geo_cache_success_wave2 <- read_csv("geo_cache_success_wave2.csv", show_col_types = FALSE)

# Combine and de-duplicate
geo_cache_final <- bind_rows(
  geo_cache_success_wave2,
  geo_results_wave3
) %>%
  distinct(country, city, .keep_all = TRUE)
...
df_all_geocoded <- df_all %>%
  left_join(geo_cache_final, by = c("country", "city")) %>%
  mutate(
    latitude = coalesce(latitude.x, latitude.y),
    longitude = coalesce(longitude.x, longitude.y)
  ) %>%
