...
if (file.exists(cache_path)) {
  ...
  geo_cache <- tibble(country = character(), city = character(), latitude = numeric(), longitude = numeric())
}
...
to_geocode <- anti_join(missing_coords, geo_cache, by = c("country", "city"))
...
if (nrow(to_geocode) > 0) {
  ...
  geo_cache <- bind_rows(geo_cache, geo_results)

  geo_cache_unique <- geo_cache %>%
...
df_all_geocoded <- df_all %>%
  left_join(geo_cache_unique, by = c("country", "city")) %>%
  mutate(
...
df_all_geocoded <- df_all %>%
  left_join(geo_cache, by = c("country", "city")) %>%
  mutate(
    latitude = coalesce(latitude.x, latitude.y),
    longitude = coalesce(longitude.x, longitude.y)
  ) %>%
