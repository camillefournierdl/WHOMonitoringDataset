...
setwd("~/Library/CloudStorage/Dropbox/0. Learning/2023 ISTP ETH Zurich/0. MSc Research Assistants/SNIS - Air Pollution/WHOMonitoringDataset-main")
...
df_2011 <- read_delim("datasetWHO2011.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
  mutate(source_year = 2011)

df_2014 <- read_delim("datasetWHO2014.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
  mutate(source_year = 2014)

df_2016 <- read_delim("datasetWHO2016.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
  mutate(source_year = 2016)

df_2018 <- read_delim("datasetWHO2018.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
  mutate(source_year = 2018)

df_2022 <- read_delim("datasetWHO2022.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
  mutate(source_year = 2022)

df_2024 <- read_delim("datasetWHO2024.csv", delim = ";", show_col_types = FALSE) %>%
  clean_names() %>%
...
#confirmed_map <- readr::read_csv("manual_city_review_confirmed.csv")
